...
class DiGraph():
    ...
    def get_node(self, key: int) -> NodeData:  # the ' -> NodeData ' is to tell which kind of element will return
    ...
    def get_edge(self, src: int, dest: int) -> EdgeData:
        if self._nodes_dict.get(src) is None:  # if the node with the key of src don't exist in nodes_dict return null
            return None
        node_source = NodeData.NodeData(self.get_node(src))  # This is how you set a type of var
        return node_source.get_edge(dest)

    def add_node(self, node: NodeData) -> None:
        key_node = NodeData.NodeData(node).get_key()  # holds the key of the node in the argument
    ...
    def connect(self, src: int, dest: int, weight: float) -> None:
        ...
        if src != dest or weight < 0 :
            ...
            if src not in self._nodes_dict:
                node_source = NodeData.NodeData(self.get_node(src))
                node_dest = NodeData.NodeData(self.get_node(dest))
                if node_source.has_Ni(dest) : 
